...
class UserService extends Service{
    ...
    try {
        ...
        if (result){
            let isMatch=await this.app.decryptPw(password,result.password);
            if (isMatch){
    ...
    //用户注册service操作
    async registor(){
        let {username,password,e_mail,mobile}=this.ctx.request.body;
        ...
        let result=await this.app.model.User.findOne({
            ...
            if (!result){
             let result_hash=await this.app.encryptPw(FACTORY_STRENTTH,password)
    ...
    async resetPass(){
        ...
        try {
            ...
            if (result){
                ...
                let result_reset=await this.app.mysql.update('users',{password:hash},{where:{mobile}});
